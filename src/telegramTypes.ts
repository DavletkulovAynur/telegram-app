//FIXME: библиотека @types/telegram-web-app не поддерживает типизацию
...
interface WebApp {
  ...
  setHeaderColor(
    color: "bg_color" | "secondary_bg_color" | (string & object),
  ): void;
  /**
...
interface CloudStorage {
  ...
  setItem(
    key: string,
    value: string,
    callback?: (error: string | null, success: null | true) => void,
  ): CloudStorage;
  /**
  ...
  getItem(
    key: string,
    callback?: (error: string | null, value: null | string) => void,
  ): CloudStorage;
  /**
   ...
   */
  getKeys(
    callback?: (error: string | null, keys: null | string[]) => void,
  ): CloudStorage;
}
